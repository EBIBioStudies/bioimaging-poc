...
public class FileMapper {
    ...
    public FilePagingResponse getFilePagingResponse(DataPage<FileAttView> files, Map<String, FileProperty> propertyMap) {
    ...
    private List<FileDTO> getFileDTOList(List<FileAttView> files, Map<String, FileProperty> metadata) {
        return files.stream().map(file -> getFileDTO(file, metadata)).collect(Collectors.toList());
    }

    private FileDTO getFileDTO(FileAttView file, Map<String, FileProperty> metadata) {
